...
summary: Spring Boot와 Socket을 사용하여 간단한 채팅 예제를 구현해봅니다.
tag: AI
---

> 해당 포스트는 한국디지털미디어고등학교 공업일반 프로젝트 수행평가를 목적으로 작성되었습니다. [자기계발계획서 확인하기](posts/springboot-socket/자기계발계획서.pdf)
...
![spring initializr](posts/springboot-socket/init.webp)

## Supabase Database 연결하기

채팅 앱에서 회원 식별과 채팅 내용 저장 등의 기능을 위해 데이터베이스를 생성한 후 연결해줘야 합니다. 우선 Supbase에 접속합니다.

![Supabase Dashboard](posts/springboot-socket/db-dash.webp)

그리고 새 프로젝트를 생성합니다. 서버를 가까운 곳으로 선택하고, 비밀번호는 기억해놓아야 합니다.

![Supabase New Project](posts/springboot-socket/db-set.webp)

그리고 프로젝트가 생성되면 아래와 같은 페이지로 이동합니다. 이 페이지가 방금 만든 프로젝트의 대시보드입니다.

![Supabase Project Dashboard](posts/springboot-socket/db-home.webp)
...
![build.gradle](posts/springboot-socket/intelij-dependencies.webp)

이 처럼 build.gradle에 위 세 가지 라이브러리를 추가해줍니다. 그리고 설치해줍니다.

<p>
  <img
    src="posts/springboot-socket/gradle-download.webp"
    alt="gradle download"
    width="200px"
  />
...
![Supabase Connect](posts/springboot-socket/db-connect.webp)
...
![application.properties](posts/springboot-socket/intelij-properties.webp)

추가로 InteliJ Ultimate 버전을 사용하신다면 아래와 같이 InteliJ와 Database를 연결해서 Database를 쉽게 확인할 수 있습니다.

![InteliJ Database 연결](posts/springboot-socket/intelij-dbcheck.webp)

## Member 관련 기능 만들기

우선 채팅 기능 구현 전 사용자들 간 식별을 위해 Member 테이블을 생성한 뒤 회원가입과 로그인 기능을 만들어 주겠습니다.

![Member Class 생성](posts/springboot-socket/member-class.webp)

위와 같이 Member Class를 생성한 뒤 프로젝트를 실행한다면 Supabase에 Member 테이블이 생성됩니다.

![Supabase Member Table](posts/springboot-socket/db-member.webp)
...
![Member Repository](posts/springboot-socket/intelij-memberrepo.webp)
...
![Security Config](posts/springboot-socket/intelij-securityconfig.webp)
...
![Member Controller](posts/springboot-socket/intelij-membercontroller.webp)

Postman으로 회원가입과 로그인을 시도해보면 Table에 잘 추가되는 것을 확인할 수 있습니다.

![Postman Register](posts/springboot-socket/postman-register.webp)

![Table에 추가된 Member](posts/springboot-socket/db-register.webp)

![Postman Login](posts/springboot-socket/postman-login.webp)
...
![Frontend Register](posts/springboot-socket/fe-register.webp)

![Frontend Register Response Ok](posts/springboot-socket/fe-registerok.webp)

![Supabase Register Response Ok](posts/springboot-socket/db-registerok.webp)

### Login

![Frontend Login](posts/springboot-socket/fe-signin.webp)

![Frontend Login Reponse Ok](posts/springboot-socket/fe-signinok.webp)
...
![Socket을 적용하지 않은 경우](posts/springboot-socket/not-socket.gif)
...
![Socket을 적용한 경우](posts/springboot-socket/apply-socket.gif)
